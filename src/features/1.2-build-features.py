...
#
# In this version, I dropping the target column 
# from testing data before saving the features.
# I am also removing any columns which are 
# completely empty from the dataset. 
...
def build_features():
    ...
    with timer('Writing training data'):
        ...
        train.to_csv(path_to_output + '1.2-features-train.csv', compression='gzip')

    with timer('Writing testing data'):
        test = dataset.loc[dataset.test == 1]
        test.drop(columns=['TARGET'], inplace=True)
        test.to_csv(path_to_output + '1.2-features-test.csv', compression='gzip')
